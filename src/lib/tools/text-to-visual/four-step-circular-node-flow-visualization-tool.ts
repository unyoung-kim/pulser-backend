...
export const fourStepCircularNodeFlowVisualizationTool = () =>
  tool({
    description: `
    This template represents a 'Four-Step Circular Node Flow Visualization' designed to visually depict a sequential or connected process divided into four stages. The layout includes:  
    1. A main header at the top for the overall title, with a sub-header below for additional context.  
    2. Four circular nodes arranged horizontally, each containing an icon to represent the step.  
    3. Curved, semi-circular lines around each node to emphasize the stage.  
    4. Vertical arrows pointing downward from each node to connect to rectangular text boxes.  

    Each text box below the nodes includes:  
    - A title summarizing the step.  
    - A description of upto three lines providing additional details about the step.  

    This template is ideal for presenting processes, schedules, or workflows with a focus on both visual clarity and step-specific details.
    `,
    parameters: z.object({
      header: z
        .string()
    ...
    execute: async (input) => {
      return {
        ...
        title_4_desc_line_3: "",
        ...input,
        template_name: "four-step-circular-node-flow-visualization",
