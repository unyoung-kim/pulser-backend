...
export const segmentedCircularVisualizationTool = () =>
  tool({
    description: `
        This tool collects parameters for a 'Segmented Circular Visualization' template and outputs them with the template name.

        Purpose:
        - Visualizes a segmented circular representation of processes, ideas, or interconnected concepts.
        - Emphasizes how the segments work together in a cyclical or interconnected manner.

        Structure:
        - A main header and sub-header at the top summarizing the diagram.
        - A segmented circular ring divided into three equal parts, each representing a concept or stage.
        - Each segment contains:
        - An icon visualizing the concept.
        - A title summarizing the segment.
        - A description block positioned externally with up to three lines for additional details.
        - Lines connect each segment to its corresponding description block.

        Parameters:
        - Main header and sub-header summarize the diagram.
        - Each segment includes an icon, title, and corresponding description lines.
    `,
    parameters: z.object({
      header: z
        .string()
    ...
    execute: async (input) => {
      return {
        ...
        title_3_desc_line_3: "",
        ...input,
        template_name: "segmented-circular-visualization",
