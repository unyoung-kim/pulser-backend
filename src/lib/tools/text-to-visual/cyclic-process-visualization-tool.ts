...
export const cyclicProcessVisualizationTool = () =>
  tool({
    description: `
      This tool collects parameters based on a "Cyclic Process Visualization" template and outputs them with the template name.

      Purpose:
      - Visualizes a continuous or cyclic process with interconnected stages.

      Structure:
      - A main header and sub-header at the top to summarize the overall concept.
      - A central circular diagram with four interconnected arrows representing stages of a cycle.
      - Each stage includes:
        - A Lucide icon inside a square block.
        - A numeric identifier within the diagram.
        - A descriptive text block outside the diagram with up to four description lines.

      Parameters:
      - Main header and sub-header summarize the concept.
      - Each stage includes a descriptive text block, label, and icon.
    `,
    parameters: z.object({
      header: z
        .string()
    ...
    execute: async (input) => {
      return {
        ...
        title_4_desc_line_4: "",
        ...input,
        template_name: "cyclic-process-visualization",
