...
export const linearMilestoneVisualizationTool = () =>
  tool({
    description: `
        This tool collects parameters for a 'Linear Milestone Visualization' template and outputs them with the template name.

        Purpose:
        - Visualizes a sequence of milestones or steps along a linear path, emphasizing progression or development over time.

        Structure:
        - A main header and sub-header at the top summarizing the diagram.
        - A linear path with connected nodes representing milestones or steps.
        - Each node contains:
        - An icon visualizing the milestone.
        - A title summarizing the milestone.
        - A corresponding description block positioned below or beside the node with up to four lines for additional details.

        Parameters:
        - Main header and sub-header summarize the sequence.
        - Each milestone includes an icon, title, and corresponding description lines.
    `,
    parameters: z.object({
      header: z
        .string()
    ...
    execute: async (input) => {
      return {
        ...
        title_4_desc_line_4: "",
        ...input,
        template_name: "linear-milestone-visualization",
