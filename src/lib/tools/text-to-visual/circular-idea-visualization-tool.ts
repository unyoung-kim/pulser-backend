...
export const circularIdeaVisualizationTool = () =>
  tool({
    description: `
    This tool collects parameters for a 'Circular Idea Visualization' template and outputs them with the template name.

    Purpose:
    - Visualizes interconnections between a central idea and related concepts or stages in a cyclical process.

    Structure:
    - A main header and sub-header at the top summarizing the diagram.
    - A central circle representing the core concept or central stage.
    - Four surrounding segments forming a circular flow, each representing a related idea or stage.
    - Each segment contains:
    - An icon visualizing the idea.
    - A title summarizing the idea.
    - A corresponding description block positioned externally with up to two lines for additional details.

    Parameters:
    - Main header and sub-header summarize the concept.
    - The central circle has a label describing the core concept.
    ...
    execute: async (input) => {
      return {
        ...
        title_4_desc_line_2: "",
        ...input,
        template_name: "circular-idea-visualization",
