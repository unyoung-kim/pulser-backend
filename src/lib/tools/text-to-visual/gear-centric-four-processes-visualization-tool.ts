...
export const gearCentricFourProcessesVisualizationTool = () =>
  tool({
    description: `
  This template represents a 'Gear-Centric Process with Four Supporting Highlights' visualization.
  It is designed to illustrate a central concept (represented by a gear) with four supporting elements around it. The layout includes:  
  1. A main header at the top for the title, accompanied by a sub-header for additional context.  
  2. A central 3D gear graphic, symbolizing the core process or system.  
  3. Four supporting blocks arranged evenly around the gear in a circular pattern.  

  Each supporting block consists of:  
  - A rectangular shape containing an icon to represent the feature.  
  - A title positioned beside the icon.  
  - A description of upto three lines below the title for additional details.  

  The background features subtle line art and symbols to reinforce the theme of technology or systems. This template is ideal for showcasing processes, systems, or components connected to a central mechanism or idea.
    `,
    parameters: z.object({
      header: z
        .string()
    ...
    execute: async (input) => {
      return {
        ...
        title_4_desc_line_3: "",
        ...input,
        template_name: "gear-centric-four-processes-visualization",
