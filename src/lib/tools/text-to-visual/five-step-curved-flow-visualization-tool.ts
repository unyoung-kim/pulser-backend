...
export const fiveStepCurvedFlowVisualizationTool = () =>
  tool({
    description: `
        This tool collects parameters for a 'Five-Step Curved Flow Visualization' template and outputs them with the template name.

        Purpose:
        - Visualizes a sequential process or workflow with five distinct steps arranged in a curved, visually appealing flow.
        - Highlights progression using numbered steps, icons, and descriptive text blocks.

        Structure:
        - A main header and sub-header at the top summarizing the visualization.
        - Five steps arranged in a curved flow, each represented by:
        - A number indicating the step order.
        - An icon visualizing the step.
        - A description block positioned below each step with up to two lines for additional details.
        - Arrows connecting the steps to illustrate progression.

        Parameters:
        - Main header and sub-header summarize the overall process.
        - Each step includes a number, icon, and corresponding description lines.
    `,
    parameters: z.object({
      header: z
        .string()
        .describe("The main title of the five-step curved flow visualization."),
      sub_header: z
        .string()
        .describe(
    ...
    execute: async (input) => {
      return {
        ...
        desc_5_line_2: "",
        ...input,
        template_name: "five-step-curved-flow-visualization",
